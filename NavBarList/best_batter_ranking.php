<?php
// NavBarList/best_batter_ranking.php
require_once '../includes/db.php';

$page_title = "Best Batter Ranking";
require_once '../includes/header.php';

// Fetch Best Batters with Scoring Rules
try {
    $stmt = $pdo->query("
        SELECT 
            u.id as player_id, 
...
<style>
    ...
    .ranking-row:hover {
        ...
        box-shadow: inset 4px 0 0 #065f46;
    }
    ...
    .player-card {
        background:
            ...
            linear-gradient(145deg, #ffffff 0%, #f8fafc 48%, #ecfdf5 100%);
    ...
    @media (max-width: 767px) {
        ...
        .alert.alert-info .d-flex.align-items-center>div {
            flex: 1;
        }
    ...
    <div class="row justify-content-center">
        <div class="col-xl-10">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div class="pe-2">
                    <h2 class="fw-bold mb-1" style="color: #065f46;">
                        <i class="fas fa-baseball-bat-ball me-2"></i>Best Batter Ranking
                    </h2>
                    <p class="text-muted mb-0">Ranked by Performance Score (Runs + Bonuses)</p>
                </div>
                <div class="badge bg-success px-3 py-2 rounded-pill shadow-sm flex-shrink-0">
            ...
            <div class="alert alert-info border-0 shadow-sm mb-4"
                style="border-radius: 15px; background: rgba(209, 231, 221, 0.5); backdrop-filter: blur(5px);">
                <div class="d-flex align-items-center">
                    <i class="fas fa-info-circle me-3 fs-4 text-success"></i>
                    <div>
                        <small class="text-dark fw-bold">Scoring System:</small>
                        <small class="text-muted d-block">Runs (+1) | SR 160+ (+15), 140+ (+10), 120+ (+5) | Fours (+1),
                            Sixes (+2) | 100+ (+20), 50+ (+12), 30+ (+5) | Duck (-5) | Win (+5)</small>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table table-hover mb-0 align-middle">
                            <thead>
                                <tr
                                    ...
                                    <th class="py-3 text-center">Runs</th>
                                    <th class="py-3 text-center">Balls</th>
                                    <th class="py-3 text-center">Score</th>
                                    <th class="pe-4 py-3 text-center" style="border-top-right-radius: 20px;">Strike Rate
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (empty($rankings)): ?>
                                    <tr>
                                        <td colspan="6" class="text-center py-5">
                                            ...
                                            <h5 class="text-muted">No batting stats available</h5>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($rankings as $index => $row):
                                        $rank = $index + 1;
                                        $strike_rate = $row['total_balls'] > 0 ? ($row['total_runs'] / $row['total_balls']) * 100 : 0;
                                        ...
                                        elseif ($rank == 3)
                                            $badge_class = 'bg-bronze text-white'; // Custom bronze below
                                        ...
                                        <tr class="ranking-row <?= (isset($_SESSION['user_id']) && $_SESSION['user_id'] == $row['player_id']) ? 'highlighted-row' : '' ?>"
                                            ...
                                            <td class="text-center">
                                                <span class="fw-bold text-success fs-5">
                                                    <?= number_format($row['best_batter_score'], 0) ?>
                                                </span>
                                            </td>
                                            <td class="text-center pe-4">
                                                <span class="badge bg-primary rounded-pill px-3 py-2">
                                                    <?= number_format($strike_rate, 2) ?>
                                                </span>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
